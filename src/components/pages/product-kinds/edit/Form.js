...
// import { updateProductKind } from 'src/graphql/mutations';
...
function Form({ className, productKind, ...rest }) {
  ...
  const methods = useForm({
    ...
    defaultValues: {
      name: {
        arEG: productKind?.name.arEG || '',
        enUS: productKind?.name.enUS || '',
      },
      status: productKind?.status == 'ACTIVE' ? true : false,
      submitError: '',
    },
  });
  const {
  ...
  async function onSubmit(values) {
    try {
      ...
      enqueueSnackbar('Product kind updated.', { variant: 'success' });
    } catch (err) {
      // Show error message
