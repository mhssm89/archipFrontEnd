...
const validationSchema = yup.object().shape({
  CategoryNameEdit: yup.string().max(255).required('Required.'),
});
...
function EditDialog({
  ...
  const methods = useForm({
    ...
    defaultValues: {
      CategoryNameEdit: editCategory.category,
    },
  });
  const {
    handleSubmit,
  ...
  // ##################################################
  async function onSubmit({ CategoryNameEdit }) {
    try {
      ...
      setCategory([
        ...category.filter((item) => item.id != editCategory.id),
        (await res).data,
      ]);
      seteditdialoug(false);
      ...
      // Show error message
      enqueueSnackbar('Error edit customer Category.', {
        variant: 'error',
      });
