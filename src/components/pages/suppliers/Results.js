...
const filterOptions = [
  { label: 'Supplier Name', id: 'supplierName' },
  { label: 'Phone Number', id: 'phonenumber' },
  { label: 'Email', id: 'emailaddress' },
];
...
function Results({ className, query, setquery, ...rest }) {
  ...
  async function resetFilter() {
    try {
      axios
        .get('http://localhost:1337/suppliers?_where[isDeleted]=0')
        .then((res) => {
          setquery(res.data);
        });
    } catch {
  ...
  async function onSubmit({ filter, search }) {
    try {
      ...
      axios
        .get(
          `http://localhost:1337/suppliers?_where[${filter.id}_contains]=${search}&[isDeleted]=0`,
        )
        .then((res) => {
  ...
  function handelDialog(supplier) {
  ...
  async function deletesupplier() {
    axios
      .put(`http://localhost:1337/suppliers/${deleteItem.id}`, { isDeleted: 1 })
      .then((deletedItem) => {
